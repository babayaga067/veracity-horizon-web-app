...
import ForgetForm from "@/app/(auth)/_components/ForgetForm";
    ...
    <div className="min-h-screen w-full flex font-sans antialiased animated-gradient">
      <div className="hidden lg:flex lg:w-5/12 relative overflow-hidden flex-col justify-between p-12">
        ...
        <div className="relative z-10 max-w-lg mt-auto mb-auto">
          <span className="inline-block py-1 px-3 rounded-full bg-white/10 text-indigo-200 text-xs font-semibold tracking-wider uppercase mb-6 border border-white/20">Password Recovery</span>
          <h1 className="text-4xl lg:text-5xl font-bold tracking-tight text-white leading-tight mb-6">Reset your password.</h1>
          <p className="text-indigo-100 text-lg leading-relaxed">Enter your email and we will send you a link to create a new password.</p>
        </div>
        ...
        <div className="w-full max-w-md mx-auto my-auto">
          ...
          <div className="glass-card rounded-3xl p-8">
            <div className="mb-8">
              ...
              <p className="text-slate-500 text-base">Enter your email to receive a password reset link.</p>
            </div>
            {submitted && (
              <div className="p-4 rounded-xl mb-6 border-2 bg-emerald-50 border-emerald-200 text-emerald-800">
                <p className="text-sm font-bold">Password reset link sent. Please check your email.</p>
              </div>
            )}
            <ForgetForm onSuccess={() => setSubmitted(true)} />
          </div>
          <div className="mt-8 text-center text-slate-600">
            Remember your password?{" "}
            <Link href="/login" className="text-indigo-600 font-bold hover:text-indigo-700 hover:underline transition-all">Sign in</Link>
          </div>
        </div>
      </div>
    </div>
