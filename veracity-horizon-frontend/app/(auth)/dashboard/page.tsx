...
import { useDebounce } from "@/app/(auth)/_components/useDebounce";
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [auctionsRes, bidsRes] = await Promise.all([
        fetchAuctionsAction(1, 100, debouncedSearch, statusFilter === "all" ? "" : statusFilter),
        handleGetMyBids(),
      ]);
      ...
      <main className="ml-64 min-h-screen relative z-10">
        <div className="max-w-7xl mx-auto px-8 py-8 space-y-8">
          <div>
            ...
            <p className="section-subtitle mt-2">Here&apos;s what&apos;s happening in your auctions today.</p>
          </div>
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="glass-card rounded-3xl border border-white/50 p-6 hover:shadow-xl hover:shadow-amber-500/10 transition-all duration-300 group">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xs font-bold text-amber-700 bg-amber-50 px-3 py-1.5 rounded-full border border-amber-100">You</span>
              </div>
              <p className="text-3xl font-bold text-slate-900">{myListings}</p>
              <p className="text-xs text-slate-500 mt-1 font-medium">My Listings</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
              <div className="glass-card rounded-3xl border border-white/50 overflow-hidden">
                <div className="px-6 py-5 border-b border-white/10 flex items-center justify-between bg-white/30">
                  <div>
                    <h3 className="section-title">Recent Auctions</h3>
                    <p className="section-subtitle mt-1">Latest listings across all categories</p>
                  </div>
                  <Link href="/market" className="text-sm font-bold text-indigo-600 hover:text-indigo-700 transition-colors">
                    View all
                  </Link>
                </div>
                <div className="px-6 py-4 flex flex-col sm:flex-row gap-3 border-b border-white/30">
                  <input
                    type="text"
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    placeholder="Search auctions..."
                    className="flex-1 px-4 py-2.5 rounded-xl border border-white/50 bg-white/50 text-sm focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all"
                  />
                  <select
                    value={statusFilter}
                    onChange={(e) => setStatusFilter(e.target.value)}
                    className="px-4 py-2.5 rounded-xl border border-white/50 bg-white/50 text-sm focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all"
                  >
                ...
                {isLoading ? (
                  ...
                  <div className="divide-y divide-white/50">
                    {auctions.slice(0, 8).map((item) => (
                      <div key={item._id} className="flex items-center gap-4 p-4 hover:bg-white/40 transition-colors">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3 mt-2">
                            <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full border ${
                              item.status === "active" || item.status === "open" ? "bg-emerald-50 text-emerald-700 border-emerald-100" :
                              item.status === "closed" ? "bg-slate-100 text-slate-600 border-slate-200" : "bg-blue-50 text-blue-700 border-blue-100"
                            }`}>
                        ...
                        <Link href={`/dashboard/auctions/${item._id}`} className="px-4 py-2 text-xs font-bold text-blue-600 border border-blue-200 rounded-xl hover:bg-blue-50 transition-all flex-shrink-0">
                          View
                        </Link>
                      </div>
                    ))}
              ...
              {recentMyBids.length > 0 && (
                <div className="glass-card rounded-3xl border border-white/50 overflow-hidden">
                  <div className="px-6 py-5 border-b border-white/10 bg-white/30">
                    <h3 className="section-title">Your Recent Bids</h3>
                    <p className="section-subtitle mt-1">Latest bidding activity across auctions</p>
                  </div>
                  <div className="divide-y divide-white/50">
                    {recentMyBids.map((auction) => {
                      ...
                      return (
                        <div key={auction._id} className="flex items-center gap-4 p-4 hover:bg-white/40 transition-colors">
                          ...
                          <Link href={`/dashboard/auctions/${auction._id}`} className="px-4 py-2 text-xs font-bold text-blue-600 border border-blue-200 rounded-xl hover:bg-blue-50 transition-all flex-shrink-0">
                            View
                          </Link>
                        </div>
                      );
            ...
            <div className="space-y-6">
              <div className="glass-card rounded-3xl border border-white/50 p-6">
                ...
                <p className="section-subtitle mt-1">Frequently used shortcuts</p>
                <div className="section-divider"></div>
                <div className="space-y-3">
                  <Link href="/dashboard/auctions/create" className="flex items-center gap-3 p-3 rounded-xl border border-white/30 hover:bg-white/50 transition-all group">
                    ...
                    <div>
                      <span className="font-bold text-slate-900 text-sm">Create Auction</span>
                      <p className="text-xs text-slate-500">Start a new listing</p>
                    </div>
                  </Link>
                  <Link href="/market" className="flex items-center gap-3 p-3 rounded-xl border border-white/30 hover:bg-white/50 transition-all group">
                    ...
                    <div>
                      <span className="font-bold text-slate-900 text-sm">Browse Market</span>
                      <p className="text-xs text-slate-500">Explore all auctions</p>
                    </div>
                  </Link>
                  <Link href="/portfolio" className="flex items-center gap-3 p-3 rounded-xl border border-white/30 hover:bg-white/50 transition-all group">
                    ...
                    <div>
                      <span className="font-bold text-slate-900 text-sm">My Portfolio</span>
              ...
              <div className="glass-card rounded-3xl border border-white/50 p-6">
                ...
                <p className="section-subtitle mt-1">Distribution of your auctions</p>
                <div className="section-divider"></div>
              ...
              <div className="glass-card rounded-3xl border border-white/50 p-6">
                ...
                <p className="section-subtitle mt-1">Your auction performance</p>
                <div className="section-divider"></div>
                <div className="space-y-4">
                  <div className="flex items-center justify-between py-2.5 border-b border-white/30">
                    <span className="text-sm text-slate-600 font-medium">Total Auctions</span>
                    <span className="font-bold text-slate-900">{auctions.length}</span>
                  </div>
                  <div className="flex items-center justify-between py-2.5 border-b border-white/30">
                    <span className="text-sm text-slate-600 font-medium">Active Bids</span>
                    <span className="font-bold text-slate-900">{totalBids}</span>
                  </div>
                  <div className="flex items-center justify-between py-2.5 border-b border-white/30">
                    <span className="text-sm text-slate-600 font-medium">Your Listings</span>
                    <span className="font-bold text-slate-900">{myListings}</span>
                  </div>
